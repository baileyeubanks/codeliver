...
async function handlePost(
  ...
  let command;
  try {
    command = parseProjectCallSheetGenerateRequest(body);
  } catch (error) {
  ...
  let result;
  try {
    result = await supabase.rpc("generate_project_call_sheet_revision", {
      p_project_id: projectId,
      p_expected_authority_version: command.expectedAuthorityVersion,
      p_expected_production_schedule_revision_id:
        command.expectedProductionScheduleRevisionId,
      p_schedule_day_id: command.scheduleDayId,
  ...
  const receipt = parseProjectCallSheetGenerateReceipt(result.data);
  if (
    !receipt ||
    receipt.projectId !== projectId ||
    receipt.source.productionScheduleRevisionId !==
      command.expectedProductionScheduleRevisionId ||
    receipt.source.scheduleDayId !== command.scheduleDayId ||
