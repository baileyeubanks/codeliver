...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (
      !asset ||
      asset.id !== assetAccess.data.id ||
      asset.project_id !== assetAccess.data.project_id
    ) {
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (jobId) {
      ...
      if (!job || job.id !== jobId || !isUuid(job.asset_id)) {
        return json({ error: "Transcode resource not found" }, 404);
      }
    ...
    const { data: job, error } = await supabase
      ...
      .maybeSingle();
    if (error) return json({ error: "Transcode request is unavailable" }, 503);
    if (job && job.asset_id !== access.data.id) {
      return json({ error: "Transcode resource not found" }, 404);
    }
