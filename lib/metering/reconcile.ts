...
function rateCatalogHash(catalog: { integrityHash: string }) {
...
export function reconcileMeteringSnapshot(
  ...
  for (const quote of snapshot.quotes) {
    ...
    if (
      ...
      rateCatalogHash(quote.rateCatalog) !== quote.rateCatalogHash ||
      quote.pricingTerms.version !== quote.pricingVersion ||
      sha256(quote.pricingTerms) !== quote.pricingTermsHash
    ) {
      add(
        "quote_pricing_lineage_mismatch",
        "Quote rate or pricing snapshot does not match its declared lineage",
        quote.id,
      );
    }
  ...
  for (const receipt of snapshot.receipts) {
    ...
    for (const eventId of receipt.ledgerEventIds) {
      ...
      if (
        ...
        event.operationExecutionId !== receipt.operationExecutionId ||
    ...
    const linkedReservation = reservationsById.get(receipt.reservationId);
    if (
      linkedReservation &&
      (receipt.operationExecutionId !== linkedReservation.operationExecutionId ||
        receipt.operation !== linkedReservation.operation ||
        receipt.rateVersion !== linkedReservation.rateVersion ||
        receipt.rateCatalogHash !== linkedReservation.rateCatalogHash ||
        receipt.pricingVersion !== linkedReservation.pricingVersion ||
        receipt.pricingTermsHash !== linkedReservation.pricingTermsHash)
    ) {
      add(
        "receipt_reservation_lineage_mismatch",
        "Receipt does not match its reservation execution or pricing lineage",
        receipt.id,
      );
    }
  ...
  const reservationsByExecution = new Map<string, string>();
  const commitsByExecution = new Map<string, string>();
  for (const reservation of snapshot.reservations) {
    ...
    if (!quote) {
      ...
      quote.operationExecutionId !== reservation.operationExecutionId ||
      quote.operation !== reservation.operation ||
      quote.rateVersion !== reservation.rateVersion ||
      quote.rateCatalogHash !== reservation.rateCatalogHash ||
      quote.pricingVersion !== reservation.pricingVersion ||
      quote.pricingTermsHash !== reservation.pricingTermsHash
    ) {
    ...
    if (
      ...
      rateCatalogHash(reservation.rateCatalog) !== reservation.rateCatalogHash ||
      reservation.pricingTerms.version !== reservation.pricingVersion ||
      sha256(reservation.pricingTerms) !== reservation.pricingTermsHash
    ) {
      add(
        "reservation_pricing_lineage_mismatch",
        "Reservation rate or pricing snapshot does not match its declared lineage",
        reservation.id,
      );
    }
    const priorReservationId = reservationsByExecution.get(
      reservation.operationExecutionId,
    );
    if (priorReservationId && priorReservationId !== reservation.id) {
      add(
        "duplicate_operation_execution_reservation",
        "Operation execution has more than one reservation",
        reservation.id,
      );
    }
    reservationsByExecution.set(reservation.operationExecutionId, reservation.id);
    ...
    for (const commit of commits) {
      const priorCommitId = commitsByExecution.get(reservation.operationExecutionId);
      if (priorCommitId && priorCommitId !== commit.id) {
        add(
          "duplicate_operation_execution_commit",
          "Operation execution has more than one customer debit",
          commit.id,
        );
      }
      commitsByExecution.set(reservation.operationExecutionId, commit.id);
      if (
        commit.operationExecutionId !== reservation.operationExecutionId ||
        commit.rateCatalogHash !== reservation.rateCatalogHash ||
        commit.pricingTermsHash !== reservation.pricingTermsHash
      ) {
        add(
          "commit_pricing_lineage_mismatch",
          "Commit receipt does not match its execution reservation lineage",
          commit.id,
        );
      }
    }
  ...
  const hasIdempotency = (
    ...
    snapshot.idempotencyRecords.some(
      (record) =>
  ...
  for (const quote of snapshot.quotes) {
    if (!hasIdempotency("estimate", "quote", quote.id, quote.requestHash)) {
      add(
        "quote_idempotency_missing",
        "Quote is missing its immutable idempotency record",
        quote.id,
      );
    }
  }
  for (const reservation of snapshot.reservations) {
    if (!hasIdempotency("reserve", "reservation", reservation.id, reservation.requestHash)) {
      add(
        "reservation_idempotency_missing",
        "Reservation is missing its immutable idempotency record",
        reservation.id,
      );
    }
  }
      ...
      receipt.kind === "collaboration"
        ...
        : receipt.kind === "commit"
          ...
          : receipt.kind === "release"
            ? "release"
            : receipt.kind === "expiration"
              ? "expire"
              : null;
    if (
      action &&
      !hasIdempotency(action, "receipt", receipt.id, receipt.requestHash)
    ) {
      add(
        "receipt_idempotency_missing",
        "Terminal receipt is missing its immutable idempotency record",
        receipt.id,
      );
    }
  }
