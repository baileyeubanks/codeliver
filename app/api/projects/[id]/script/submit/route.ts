...
async function handlePost(
  ...
  let mutation;
  try {
    mutation = parseProjectScriptSubmitRequest(body);
  } catch (error) {
  ...
  let result;
  try {
    result = await supabase.rpc("submit_project_script_revision", {
  ...
  const receipt = parseProjectScriptSubmitReceipt(result.data);
  if (
    !receipt ||
