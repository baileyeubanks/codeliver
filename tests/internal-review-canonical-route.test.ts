...
test("legacy internal asset URLs resolve to the canonical cockpit review state", () => {
  ...
  assert.equal(
    internalReview.buildCanonicalInternalReviewHref("ica", "rough-cut", false, "version-4"),
    "/projects/ica?asset=rough-cut&version=version-4&view=review",
  );
});
...
test("the internal route validates before replacing into the bright project cockpit", () => {
  ...
  assert.match(
    loader,
    /buildCanonicalInternalReviewHref\([\s\S]*?identity\.projectId,[\s\S]*?identity\.assetId,[\s\S]*?requestedVersionId/,
  );
  assert.match(componentSource, /controller\.abort\(\)/);
  assert.match(componentSource, /bg-\[#f7f9fc\]/);
});
