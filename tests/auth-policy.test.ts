...
test("signup validation normalizes identity without weakening the server password contract", () => {
  assert.equal(normalizeAuthEmail("  Bailey@ContentCo-op.com "), "bailey@contentco-op.com");
  assert.deepEqual(
    validateSignupCredentials({
    ...
    { email: "bailey@contentco-op.com", error: null, field: null },
  );
  assert.match(
...
test("surface mismatch notices accept only safe managed portal instructions", () => {
  assert.deepEqual(
    ...
    ),
    {
      ...
      portalLabel: "client review portal",
  ...
  assert.deepEqual(
    ...
    ),
    {
      ...
      portalLabel: "Content Co-op team portal",
...
test("callback status notices are stable and never expose provider details", () => {
  assert.deepEqual(resolveAuthStatusNotice("?access=pending&next=%2Fprojects"), {
    kind: "pending",
    message: "Account access is pending approval.",
  });
  assert.deepEqual(
    resolveAuthStatusNotice("?auth_error=private-provider-stack&next=%2Fprojects"),
    {
      kind: "error",
      message: "Authentication could not be completed. Sign in again.",
    },
  );

  for (const search of [
    "?access=pending&access=surface_mismatch",
    "?auth_error=one&auth_error=two",
    "?access=pending&auth_error=exchange_failed",
    "?access=surface_mismatch",
  ]) {
    assert.equal(resolveAuthStatusNotice(search), null, search);
  }
});
