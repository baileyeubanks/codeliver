...
export async function POST(req: NextRequest) {
  ...
  if (asset.project_id !== ownership.data.project_id) {
    return NextResponse.json(
      {
        error: "Asset project authority changed during this request.",
        code: "ASSET_PROJECT_AUTHORITY_CHANGED",
      },
      { status: 409 },
    );
  }
  ...
  try {
    ...
    const job = await service.enqueue({
      ...
      projectId: ownership.data.project_id,
      source: {
