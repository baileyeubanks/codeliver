...
export const CO_PRODUCE_PERMISSION_CONTRACTS = {
  ...
  "sales:read": {
    authority: "workspace-rbac",
    state: "enforced",
    description: "Read team-bound inquiry, opportunity, and creative-brief context.",
  },
  "sales:qualify": {
    authority: "workspace-rbac",
    state: "enforced",
    description: "Qualify an inquiry into one account, contact, opportunity, and brief origin.",
  },
  "activity:read": {
    authority: "workspace-rbac",
    state: "enforced",
  ...
  "preproduction:write": {
    authority: "workspace-rbac",
    state: "enforced",
    description: "Author governed project plans, scripts, shot plans, schedules, call sheets, and authorized task records.",
  },
  "production:manage": {
    authority: "workspace-rbac",
    state: "planned",
...
export const CO_PRODUCE_RECORDS = {
  ...
  approval_workflow: {
    ...
    status: {
      ...
      values: ["active", "completed", "cancelled", "superseded"],
      transitionRule: "Workflow completion derives from persisted step decisions; uploading a newer media version supersedes only its active prior round.",
    },
  },
  approval_step: {
    authority: "co-deliver",
  ...
  project_operating_record: {
    ...
    scope: "project",
    parent: "project",
    writeRule: "The operating record is a role-filtered read-only projection and never accepts direct writes.",
    status: {
      kind: "derived",
      field: null,
      values: ["not_started", "blocked", "ready", "active", "complete"],
      transitionRule: "Workspace readiness is recomputed from authoritative project, proposal handoff, media, review, approval, and delivery evidence.",
    },
  },
  project_analytics: {
    authority: "co-deliver",
  ...
  project_brief_revision: {
    authority: "shared",
    deployment: "canonical",
    storage: "project_brief_revisions",
    scope: "project",
    parent: "project",
    writeRule: "Revision 1 is projected only from an authorized accepted-proposal receipt and rejects direct mutation.",
    status: {
      kind: "immutable",
      field: null,
      values: [],
      transitionRule: "The accepted brief is immutable; future authoring must append a separately authorized revision.",
    },
  },
  script_revision: {
    authority: "co-script",
    deployment: "canonical",
    storage: "project_script_revisions",
    scope: "project",
    parent: "project",
    writeRule: "Every script save appends a project-scoped revision; submission and producer decisions append durable receipts instead of mutating prior text.",
    status: {
      kind: "derived",
      field: null,
      values: ["draft", "submitted", "approved", "changes_requested", "superseded"],
      transitionRule: "The latest immutable revision is draft until receipt-backed submission and decision events exist; any older revision is superseded by derivation.",
    },
  },
  shot_plan_revision: {
    authority: "co-script",
    deployment: "canonical",
    storage: "project_shot_plan_revisions",
    scope: "project",
    parent: "project",
    writeRule: "Every generated or editor-authored shot plan is an immutable revision bound to the exact approved script and active production-plan revisions; producer decisions append durable receipts.",
    status: {
      kind: "derived",
      field: null,
      values: ["draft", "submitted", "approved", "changes_requested"],
      transitionRule: "The highest approved source-current revision is active; older revisions remain immutable history, and source changes make prior drafts stale without rewriting them.",
    },
  },
  production_schedule_revision: {
    authority: "co-script",
    deployment: "canonical",
    storage: "project_production_schedule_revisions",
    scope: "project",
    parent: "project",
    writeRule: "Every production schedule is an immutable revision bound to the exact active approved shot-plan revision and approval receipt; timing remains explicit human-entered production authority.",
    status: {
      kind: "derived",
      field: null,
      values: ["draft", "submitted", "approved", "changes_requested"],
      transitionRule: "The highest approved source-current revision is active; source changes make prior schedules stale without rewriting their timing or history.",
    },
  },
  call_sheet_revision: {
    authority: "co-script",
    deployment: "canonical",
    storage: "project_call_sheet_revisions",
    scope: "project",
    parent: "production_schedule_revision",
    writeRule: "Every call sheet is an immutable revision bound to one exact day in the active approved production schedule; producer decisions append durable receipts.",
    status: {
      kind: "derived",
      field: null,
      values: ["draft", "submitted", "approved", "changes_requested"],
      transitionRule: "The highest approved source-current revision is active for its schedule day; schedule changes make prior call sheets stale without rewriting them.",
    },
  },
  "planned.call_sheet_distribution": {
    authority: "co-deliver",
    deployment: "planned",
    storage: null,
    scope: "project",
    parent: "call_sheet_revision",
    writeRule: "Distribution requires recipient-scoped delivery, acknowledgement, and notification evidence separate from call-sheet authoring.",
    status: {
      kind: "not-implemented",
      field: null,
      values: [],
      transitionRule: "An approved call sheet is not sent or acknowledged until a governed distribution record exists.",
    },
  },
  ...
  production_plan_revision: {
    authority: "co-deliver",
    deployment: "canonical",
    storage: "production_plan_revisions",
    scope: "project",
    parent: "project",
    writeRule: "Each accepted plan is an immutable project-scoped revision with source lineage and an idempotent creation receipt.",
    status: {
      kind: "derived",
      field: "production_plan_revisions.revision_number",
      values: ["active", "superseded"],
      transitionRule: "The highest durable revision is active; replanning appends a revision and never rewrites prior plans.",
    },
  },
  production_task: {
    authority: "co-deliver",
    deployment: "canonical",
    storage: "production_tasks",
    scope: "project",
    parent: "production_plan_revision",
    writeRule: "Task fields and status change only through expected-version project commands under pre-production authority.",
    status: {
      kind: "authoritative-field",
      field: "production_tasks.status",
      values: ["todo", "in_progress", "blocked", "completed", "cancelled"],
      transitionRule: "Every accepted transition increments task authority and appends a project pre-production event.",
    },
  },
  project_preproduction_event: {
    authority: "co-deliver",
    deployment: "canonical",
    storage: "project_preproduction_events",
    scope: "project",
    parent: "project",
    writeRule: "Pre-production plan, task, script, shot-plan, and production-schedule events are append-only, hash-linked evidence emitted by committed project commands.",
    status: {
      kind: "immutable",
      field: null,
      values: [],
      transitionRule: "Corrections require another authorized command; prior event evidence is never rewritten.",
    },
  },
...
export const CO_PRODUCE_CAPABILITY_GROUPS = [
  {
    ...
    capabilities: [
      ...
      {
        id: "workspace-shell.project-operating-record",
        owner: "workspace-shell",
        label: "Project operating record",
        summary: "Carry accepted scope, timing, deliverables, media, review, and delivery readiness through one role-filtered project projection.",
        surfaces: [
          surface("project.operating-record", "Project operating record", "lifecycle-dashboard", "system-map"),
        ],
        route: { kind: "page", intent: "view-project-operating-record", routeId: "project" },
        permission: workspacePermission("projects:read"),
        data: {
          primary: "project_operating_record",
          supporting: ["project", "asset", "review", "approval_step", "activity_event"],
          authorityRule: "This projection connects source records without becoming a second project, commercial, review, or delivery authority.",
        },
        audit: readOnlyAudit("Reading or recomputing the project operating record emits no mutation event."),
        readiness: {
          state: "read-only",
          claim: "A project-scoped API returns production-safe proposal context and current lifecycle evidence filtered to the caller's project role.",
          blockers: ["Durable pre-production, production-session, and delivery records remain separate follow-on authorities."],
        },
      },
      {
    ...
    capabilities: [
      {
        ...
        label: "Project brief",
        summary: "Inspect the approved creative goals, requirements, references, and acceptance criteria.",
        surfaces: [
        ...
        permission: workspacePermission("projects:read"),
        data: {
          primary: "project_brief_revision",
          supporting: ["project", "project_operating_record", "production_plan_revision"],
          authorityRule: "The project-scoped revision projected from the authorized proposal is canonical; the operating record is a role-filtered read model.",
        },
        audit: readOnlyAudit("Reading the approved brief does not append project activity."),
        readiness: {
          state: "read-only",
          claim: "Internal project contributors can inspect the immutable accepted-proposal brief in the existing Plan workspace.",
          blockers: [
            "Writable project-brief revisions remain unavailable until an append-only authorization path is implemented.",
            "The source migration remains unapplied until staging RLS and trigger certification pass.",
          ],
        },
      },
      {
        ...
        owner: "pre-production",
        label: "Scripts",
        summary: "Write, submit, and approve immutable project scripts before visual planning begins.",
        ...
        data: {
          primary: "script_revision",
          supporting: ["project", "project_brief_revision", "project_operating_record"],
          authorityRule: "Co-Script owns immutable script revisions and decision receipts under the existing project, identity, permission, and event-head authority.",
        },
        audit: {
          responsibility: "append-after-commit",
          record: "project_preproduction_event",
          events: [
          ...
          rule: "Every accepted script command writes its immutable receipt and hash-linked project event in the same transaction.",
        },
        readiness: {
          state: "guarded",
          claim: "Internal project contributors can author and submit script revisions, while producers hold the approval decision authority.",
          blockers: [
            "The source migration remains unapplied until staging RLS, concurrency, and receipt-chain certification pass.",
            "Call-sheet distribution remains a separate unavailable authority.",
          ],
        },
      },
      {
        ...
        owner: "pre-production",
        label: "Shot plan & schedule",
        summary: "Turn the approved script into governed shots, then schedule every shot with explicit production timing.",
        ...
        permission: workspacePermission("preproduction:write"),
        data: {
          primary: "production_schedule_revision",
          supporting: ["project", "script_revision", "production_plan_revision", "shot_plan_revision", "call_sheet_revision", "planned.call_sheet_distribution"],
          authorityRule: "Co-Script owns immutable source-bound shot-plan, production-schedule, and call-sheet revisions; timing and production instructions remain explicit human input.",
        },
        audit: {
          responsibility: "append-after-commit",
          record: "project_preproduction_event",
          ...
          rule: "Every accepted shot-plan command writes its immutable receipt and hash-linked project event in the same transaction.",
        },
        readiness: {
          state: "guarded",
          claim: "Internal contributors can revise source-bound shots, explicit shoot timing, and day-specific call sheets, while producers hold generation and approval authority.",
          blockers: [
            "The source migration remains unapplied until staging RLS, concurrency, and receipt-chain certification pass.",
            "Call-sheet distribution and acknowledgement remain unavailable until a governed recipient delivery authority exists.",
          ],
        },
      },
      {
      ...
      },
      {
        ...
        route: {
          kind: "action",
          intent: "open-production-tasks",
          routeId: "project",
          action: "surface=tasks",
        },
        permission: workspacePermission("preproduction:write"),
        data: {
          primary: "production_task",
          supporting: [
            "project",
            "production_plan_revision",
            "project_preproduction_event",
          ...
          authorityRule: "Task completion is versioned project authority; media approval remains separate evidence.",
        },
        audit: {
          responsibility: "append-after-commit",
          record: "project_preproduction_event",
          events: ["production.plan.initialized", "production.task.updated"],
          rule: "The database command appends a hash-linked event and idempotency receipt in the same transaction.",
        },
        readiness: {
          state: "guarded",
          claim: "Authorized project members can read current plans and update task status through durable expected-version commands.",
          blockers: [
            "Plan creation is API-backed but does not yet have a full cockpit authoring form.",
            "The source migration remains unapplied until staging RLS and RPC certification pass.",
          ],
        },
      },
    ],
  ...
  },
  {
    ...
    capabilities: [
      ...
      },
      {
        ...
        summary: "Attach version-bound visual marks to review comments.",
        surfaces: [surface("cockpit.annotation-tools", "Select, Pen, Arrow, Text, Highlight, Eraser", "review-cockpit")],
        route: unavailableRoute("annotate-review-frame", "Annotation data types exist, but the canonical review cockpit does not expose a durable full drawing workflow."),
        ...
        audit: unavailableAudit("No full annotation-tool workflow is claimed as live."),
        readiness: unavailableReadiness("The visible multi-tool annotation surface is not durably wired end to end."),
      },
      {
