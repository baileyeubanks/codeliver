...
function storageErrorResponse(error: unknown): NextResponse | null {
...
export async function streamTrustedMediaPath({
  request,
  requestedPath,
  download = false,
  downloadName,
  ...
  try {
    ...
    const headers: Record<string, string> = {
      "Content-Type": mimeType(mediaPath.relativePath),
      "Accept-Ranges": "bytes",
    ...
    if (download) {
      const fileName = sanitizeMediaFilename(
        downloadName || basename(mediaPath.relativePath),
      );
