...
function unavailable() {
  return json({ error: "Project shot planning is temporarily unavailable" }, 503);
}

function projectAccessError(status: number) {
  if (status === 403) return json({ error: "Forbidden" }, 403);
  if (status === 404) return json({ error: "Project shot plan not found" }, 404);
  return unavailable();
}

function databaseError(error: { code?: string; message?: string } | null) {
  const mapped = classifyProjectShotPlanDatabaseError(error);
  return json({ error: mapped.error }, mapped.status);
}

function validationError(error: unknown) {
  if (error instanceof ProjectShotPlanValidationError) {
    return json(
      {
        error: error.message,
        code: error.code,
  ...
  return json({ error: "The project shot plan request is invalid" }, 422);
}
...
async function projectIdFrom(params: Promise<{ id: string }>) {
  try {
    return normalizeProjectShotPlanUuid((await params).id, "projectId");
  } catch {
    return null;
  }
}
...
async function handleGet(
  ...
  if (!projectId) return json({ error: "Project shot plan not found" }, 404);
  ...
  let result;
  try {
    result = await supabase.rpc("get_project_shot_plan", {
  ...
  const snapshot = parseProjectShotPlanSnapshot(result.data);
...
async function handlePost(
  ...
  if (
    ...
    declaredLength > PROJECT_SHOT_PLAN_APPEND_MAX_BYTES
  ) {
    return json({ error: "Project shot plan request is too large" }, 413);
  }
  const raw = await request.text();
  if (Buffer.byteLength(raw, "utf8") > PROJECT_SHOT_PLAN_APPEND_MAX_BYTES) {
    return json({ error: "Project shot plan request is too large" }, 413);
  }
  let body: unknown;
  try {
  ...
  let command;
  try {
    command = parseProjectShotPlanAppendRequest(body);
  } catch (error) {
  ...
  let result;
  try {
    result = await supabase.rpc("append_project_shot_plan_revision", {
      p_project_id: projectId,
  ...
  if (result.error) return databaseError(result.error);
  const receipt = parseProjectShotPlanAppendReceipt(result.data);
  if (
    !receipt ||
