...
DECLARE
  ...
  realtime_publication_owner oid;
...
BEGIN
  IF server_version_num < 150000 THEN
    RAISE EXCEPTION USING
      ERRCODE = '0A000',
      MESSAGE = format(
        'Co-Production authority requires PostgreSQL 15 or newer; detected server_version_num=%s.',
        server_version_num
      );
  END IF;
  ...
  IF current_setting('wal_level') <> 'logical' THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = format(
        'Co-Production Realtime requires wal_level=logical; detected wal_level=%s.',
        current_setting('wal_level')
      );
  END IF;

  SELECT publication.pubowner, publication.puballtables
  INTO realtime_publication_owner, realtime_publication_all_tables
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'Co-Production Realtime requires the supabase_realtime publication before this migration can run.';
  END IF;

  IF realtime_publication_all_tables THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'Co-Production Realtime requires a selective supabase_realtime publication; FOR ALL TABLES is not permitted.';
  END IF;

  IF NOT pg_catalog.pg_has_role(
    current_user,
    realtime_publication_owner,
    'USAGE'
  ) THEN
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = format(
        'Migration role %I must own or be a member of the owner role for publication supabase_realtime.',
        current_user
      );
  END IF;
END
...
ALTER DEFAULT PRIVILEGES IN SCHEMA co_production_private
...
CREATE TABLE co_production.folders (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT folders_id_project_key UNIQUE (id, project_id),
  CONSTRAINT folders_parent_project_fk
...
CREATE TABLE co_production.assets (
  ...
  CONSTRAINT assets_folder_project_fk
...
CREATE TABLE co_production.reviews (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT reviews_id_asset_key UNIQUE (id, asset_id),
...
CREATE TABLE co_production.comments (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT comments_id_asset_key UNIQUE (id, asset_id),
  FOREIGN KEY (version_id, asset_id)
    REFERENCES co_production.versions(id, asset_id)
    ON DELETE RESTRICT,
  CONSTRAINT comments_review_asset_fk
    FOREIGN KEY (review_id, asset_id)
    REFERENCES co_production.reviews(id, asset_id)
    ON DELETE CASCADE,
  CONSTRAINT comments_review_invite_asset_fk
    FOREIGN KEY (review_invite_id, asset_id)
    REFERENCES co_production.review_invites(id, asset_id)
    ON DELETE SET NULL (review_invite_id),
  CONSTRAINT comments_parent_asset_fk
    FOREIGN KEY (parent_id, asset_id)
    REFERENCES co_production.comments(id, asset_id)
    ON DELETE CASCADE
);
...
CREATE TABLE co_production.annotations (
  ...
  FOREIGN KEY (version_id, asset_id)
    REFERENCES co_production.versions(id, asset_id)
    ON DELETE RESTRICT,
  CONSTRAINT annotations_comment_asset_fk
    FOREIGN KEY (comment_id, asset_id)
    REFERENCES co_production.comments(id, asset_id)
    ON DELETE CASCADE
);
...
CREATE TABLE co_production.approvals (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT approvals_workflow_asset_fk
    FOREIGN KEY (workflow_id, asset_id)
    REFERENCES co_production.approval_workflows(id, asset_id)
    ON DELETE CASCADE
);
...
CREATE TABLE co_production.activity_log (
  ...
  CONSTRAINT activity_log_asset_project_fk
...
CREATE TABLE co_production.edit_decisions (
  ...
  client_request_id uuid NOT NULL DEFAULT pg_catalog.gen_random_uuid(),
  metadata jsonb NOT NULL DEFAULT '{}'::jsonb,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  ...
  FOREIGN KEY (version_id, asset_id)
    REFERENCES co_production.versions(id, asset_id)
    ON DELETE CASCADE,
  CONSTRAINT edit_decisions_review_invite_asset_fk
    FOREIGN KEY (review_invite_id, asset_id)
    REFERENCES co_production.review_invites(id, asset_id)
    ON DELETE SET NULL (review_invite_id)
);
...
BEGIN
  ...
  IF approval_count > 0 AND NOT co_production_private.has_asset_role(
    ...
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = 'Producer authority is required to reset existing approvals for a new version.';
  END IF;
  ...
  IF previous_version_id IS NOT NULL THEN
    INSERT INTO co_production.comments (
      review_id,
      asset_id,
      version_id,
      review_invite_id,
    ...
    WHERE comment.asset_id = target_asset_id
      AND comment.version_id = previous_version_id
      AND comment.status <> 'resolved'
  ...
  GET DIAGNOSTICS reset_approval_count = ROW_COUNT;
  IF reset_approval_count <> approval_count THEN
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = 'RLS prevented one or more approvals from being reset; the version was not created.';
  END IF;
...
CREATE POLICY activity_log_insert ON co_production.activity_log
  ...
  WITH CHECK (
    ...
    AND (project_id IS NULL OR co_production_private.has_project_role(project_id, 10))
    AND (asset_id IS NULL OR co_production_private.has_asset_role(asset_id, 10))
    AND (project_id IS NOT NULL OR asset_id IS NOT NULL)
  );
...
GRANT EXECUTE ON FUNCTION co_production.create_asset_version(
  uuid,
  text,
  bigint,
  text,
...
COMMENT ON FUNCTION co_production.create_asset_version(
  uuid,
  text,
  bigint,
  text,
  ...
  'Atomic SECURITY INVOKER version transition. RLS authorizes every read and write; existing approval rows require producer authority.';
