...
function reviewLinkChecks(repoRoot) {
  ...
  const passwordEnforced = /password_hash/.test(`${inviteLib}\n${publicRoute}`) && /compare|verify|argon|bcrypt|scrypt|timingSafeEqual/.test(`${inviteLib}\n${publicRoute}`);
  const passwordCheck = result(
    ...
    !passwordFieldExists || passwordEnforced ? "pass" : "fail",
    {
      ...
      evidence: [sourceEvidence(inviteLibFile, inviteLib, /token|password/, "review invite validation"), sourceEvidence(publicRouteFile, publicRoute, /token|password/, "public review access")],
  ...
  const exposesOriginal = /file_url|fileUrl|original/i.test(publicRoute) && /NextResponse\.json/.test(publicRoute);
  const watermarkPassThrough = /original|file_url|fileUrl/i.test(watermark) && !/transcode|renderWatermark|signedUrl|signed_url/i.test(watermark);
  const publicControls = result(
    "security.public-file-controls",
    "Download and watermark policy is enforced at the media-delivery boundary",
    exposesOriginal || watermarkPassThrough ? "fail" : "pass",
    {
      severity: "critical",
      summary: exposesOriginal || watermarkPassThrough ? "Public review or watermark response appears to expose/pass through the original media URL" : "No original-media bypass detected",
      evidence: [sourceEvidence(publicRouteFile, publicRoute, /file_url|fileUrl/, "public media URL"), sourceEvidence(watermarkFile, watermark, /original|file_url|fileUrl/i, "watermark response")],
      residualRisk: exposesOriginal || watermarkPassThrough ? "UI-only download controls can be bypassed and required branding may not be applied." : null,
    }
  );
...
function schemaContractCheck(repoRoot) {
  ...
  for (const file of migrationFiles) {
    const source = readFileSync(file, "utf8");
    for (const match of source.matchAll(/create\s+table\s+(?:if\s+not\s+exists\s+)?(?:public\.)?["']?([a-zA-Z0-9_]+)["']?/gi)) {
      tables.add(match[1]);
    }
  }
  ...
  const missing = [...references.keys()].filter((table) => !tables.has(table)).sort();
  return result(
    "consistency.schema-contract",
    "Runtime table references exist in repository migrations",
    missing.length === 0 ? "pass" : "fail",
    {
      severity: "critical",
      summary: missing.length === 0 ? `${references.size} referenced tables are declared` : `Missing migration declarations: ${missing.join(", ")}`,
...
function notificationAndWebhookChecks(repoRoot) {
  ...
  const targetAuthorized = /user_id\s*===\s*user\.id|requireTeamRole|authorizeRecipient|assertNotificationAuthority|getOwnedProject/.test(notification);
  const notificationCheck = result(
    "security.notification-recipient-authorization",
    "Outbound notification recipients are server-authorized",
    !acceptsTarget || targetAuthorized ? "pass" : "fail",
    {
      severity: "critical",
      summary: acceptsTarget && !targetAuthorized ? "Authenticated callers can select user_id without a detected tenant or role check" : "Recipient authority is constrained",
      evidence: [sourceEvidence(notificationFile, notification, /user_id/, "caller-selected recipient")],
      residualRisk: acceptsTarget && !targetAuthorized ? "A user can create or send a notification to another account." : null,
    }
  );
  ...
  const protections = {
    urlValidation: /new URL\(|validateWebhookUrl|assertPublicUrl/.test(webhook),
    privateNetworkBlock: /private|loopback|linkLocal|127\.0\.0\.1|169\.254|isPrivate/i.test(webhook),
    timeout: /AbortSignal\.timeout|AbortController|timeout/.test(webhook),
    hmac: /createHmac|timingSafeEqual|hmac/i.test(webhook),
    idempotency: /idempot|delivery_attempt|outbox|retry/i.test(webhook),
  };
  ...
  const webhookCheck = result(
    ...
    missing.length === 0 ? "pass" : "fail",
    {
      ...
      evidence: [{ type: "source", file: webhookFile, note: "webhook delivery path" }],
...
function productionSettingsCheck(repoRoot) {
  ...
  const source = readSource(repoRoot, file);
  const demoAuthority = /useDemoWorkspace|demoWorkspace/.test(source);
  const serverPersistence = /\/api\/|supabase|server action|useActionState/i.test(source);
  return result(
    "product.production-settings-authority",
    "Production settings use a server-backed workspace authority",
    !demoAuthority && serverPersistence ? "pass" : "fail",
    {
      severity: "critical",
      summary: demoAuthority ? "Settings are coupled to demo workspace state" : serverPersistence ? "Server-backed settings authority detected" : "No settings persistence authority detected",
      evidence: [sourceEvidence(file, source, /useDemoWorkspace|demoWorkspace|\/api\//, "settings data authority")],
      residualRisk: demoAuthority || !serverPersistence ? "Users can believe preferences or branding are saved when they exist only in one browser." : null,
    }
  );
}
...
function versionTransactionCheck(repoRoot) {
  ...
  const transaction = /\.rpc\(|transaction|serializable|advisory_lock/i.test(source);
  return result(
    "resilience.version-transaction",
...
function proofFileResult(repoRoot, journey, binding) {
  ...
  try {
    ...
    const required = ["desktop", "mobile", "accessibility", "persistence", "security", "performance"];
