...
export function buildPublicIntakeQuarantineObjectKey(input: {
  formKeyHash: string;
  objectId: string;
  version: number;
  filename: string;
}): string {
  if (!/^[0-9a-f]{64}$/.test(input.formKeyHash)) {
    throw new StorageError("STORAGE_PATH_INVALID", "Intake form namespace is invalid");
  }
  if (!Number.isSafeInteger(input.version) || input.version <= 0) {
    throw new StorageError("STORAGE_PATH_INVALID", "Object version must be a positive integer");
  }
  ...
  return assertSafeObjectKey(
    [
      "intake-quarantine",
      "forms",
      `f-${input.formKeyHash.slice(0, 32)}`,
      "uploads",
      `u-${digest(object)}`,
      version,
      sanitizeObjectFilename(input.filename),
