...
function plane(store = new InMemoryCollaborationEventStore()) {
  let id = 0;
...
function expectProblem(result, code) {
  ...
  assert.ok(result.problem.recovery.length > 0);
...
test("rejects cross-tenant injection without disclosing the foreign resource", () => {
  const store = new InMemoryCollaborationEventStore();
  const controlPlane = plane(store);
  ...
  assert.equal(store.currentSequence(foreignScope), 0);
});
...
test("tenant scopes keep independent sequence and idempotency domains", () => {
  const controlPlane = plane();
  const tenant2Scope = {
    tenantId: "tenant-2",
    projectId: "project-2",
  ...
  const tenant1 = controlPlane.execute(
    command("thread.create", { body: "Tenant one" }),
    principal(),
    resource(),
    "trace-tenant-1",
  );
  const tenant2 = controlPlane.execute(
    command("thread.create", { body: "Tenant two" }, 0, { scope: tenant2Scope }),
    principal({ actorId: "actor-2", tenantId: "tenant-2" }),
    resource({ ...tenant2Scope, currentAssetVersionId: tenant2Scope.assetVersionId }),
    "trace-tenant-2",
  );

  assert.equal(tenant1.ok, true);
  assert.equal(tenant2.ok, true);
  assert.equal(tenant1.value.event.sequence, 1);
  assert.equal(tenant2.value.event.sequence, 1);

  const tenant1Events = controlPlane.readEvents(scope, 0, 10, principal(), resource());
  const tenant2Events = controlPlane.readEvents(
    tenant2Scope,
    0,
    10,
    principal({ actorId: "actor-2", tenantId: "tenant-2" }),
  ...
  assert.equal(tenant1Events.ok, true);
  assert.equal(tenant2Events.ok, true);
  assert.deepEqual(tenant1Events.value.items.map((event) => event.scope.tenantId), ["tenant-1"]);
  assert.deepEqual(tenant2Events.value.items.map((event) => event.scope.tenantId), ["tenant-2"]);
});
