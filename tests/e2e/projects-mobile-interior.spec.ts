...
async function openProjects(
  page: Page,
  fixture: "loading" | "populated" | "empty" | "error" = "populated",
) {
...
async function expectWorkspaceDataActionsSuppressed(page: Page) {
  await page
    .getByRole("button", { name: "Search commands, projects, and media" })
    .click();
  const palette = page.getByRole("dialog", { name: "Command palette" });
  await expect(palette).toBeVisible();
  await expect(
    palette.getByRole("option", { name: /New project/ }),
  ).toHaveCount(0);
  await expect(palette.getByText("Open project cockpit")).toHaveCount(0);
  await expect(
    palette.getByRole("option", { name: /Charles Drummond_v5/ }),
  ).toHaveCount(0);
...
test.describe("Projects mobile interior geometry", () => {
  test("contains storage status without overlapping the first project card", async ({
    page,
  }) => {
    ...
    const [noticeBox, projectBox] = await Promise.all([
      notice.boundingBox(),
  ...
  test("has no horizontal overflow and leaves the last content above bottom navigation", async ({
    page,
  }) => {
    await openProjects(page);
    ...
    const workspaceMain = await page
      .locator("#workspace-content")
      .boundingBox();
    expect(workspaceMain).not.toBeNull();
    expect(Math.abs(workspaceMain!.width - MOBILE.width)).toBeLessThanOrEqual(
      1,
    );
    ...
    expect(workspaceMain!.y + workspaceMain!.height).toBeLessThanOrEqual(
      navBox!.y + 1,
    );
    expect(copilotBox!.y + copilotBox!.height).toBeLessThanOrEqual(
      navBox!.y - 8,
    );
  ...
  test("shows one coherent visible menu affordance", async ({ page }) => {
    ...
    const visibleCount = await menuAffordances.evaluateAll(
      (elements) =>
  ...
  test("keeps every visible Projects action at least 44px tall", async ({
    page,
  }) => {
    await openProjects(page);

    const undersized = await page
      .locator('[data-projects-action="true"]')
...
test.describe("Projects response states", () => {
  const projectsActions = (page: Page) =>
    page.locator('[data-projects-action="true"]');
  const shellUpload = (page: Page) =>
    page.getByRole("button", { name: "Upload media to a project" });

  test("keeps loading distinct from populated, empty, and error", async ({
    page,
  }) => {
  ...
  test("failure exposes Retry only and retry-in-flight exposes zero domain actions", async ({
    page,
  }) => {
    ...
    await expect(
      errorState.getByRole("heading", { name: "Projects unavailable" }),
    ).toBeVisible();
    await expect(
      errorState.getByRole("button", { name: "Retry" }),
    ).toBeVisible();
  ...
  test("legitimate empty exposes exactly one New project CTA", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Projects unavailable" }),
    ).toHaveCount(0);
    await expect(
      page.getByRole("link", { name: "New project", exact: true }),
    ).toHaveCount(1);
    await expect(projectsActions(page)).toHaveCount(1);
    await expect(shellUpload(page)).toHaveCount(0);
    await expectWorkspaceDataActionsSuppressed(page);
  ...
  test("populated Projects preserves its actions and demo Upload", async ({
    page,
  }) => {
    await openProjects(page);
    await expect(page.getByTestId("project-list")).toBeVisible();
  ...
  test("an open Upload dialog stays closed across unavailable and recovered states", async ({
    page,
  }) => {
    await openProjects(page);
    ...
    await page.evaluate(() => {
      window.history.pushState(
        {},
        "",
        "/projects?demo=1&projectsFixture=loading",
      );
  ...
  test("uses Sapphire Light canon tokens", async ({ page }) => {
    ...
    const tokens = await page.evaluate(() => {
      ...
      const primary = getComputedStyle(
        document.querySelector(".projects-primary-action")!,
      );
      return {
