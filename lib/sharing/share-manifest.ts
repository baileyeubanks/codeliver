...
function parseManifest(
  ...
  for (let index = 0; index < rawItems.length; index += 1) {
    ...
    const approvalId = stringValue(ownValue(item, input, "approval_id"), 128);
    if (approvalId === undefined) {
      return fail("approval_id is invalid", `${prefix}.approval_id`);
    }
    if (shareIntent === "approval_needed" && !approvalId) {
      return fail(
        "approval_id is required for approval-needed shares",
        `${prefix}.approval_id`,
      );
    }
    if (shareIntent !== "approval_needed" && approvalId) {
      return fail(
        "approval_id is only valid for approval-needed shares",
        `${prefix}.approval_id`,
      );
    }
    ...
    const rawPassword = ownValue(item, input, "password");
    const password = rawPassword == null || rawPassword === "" ? null : rawPassword;
    if (
      password !== null &&
