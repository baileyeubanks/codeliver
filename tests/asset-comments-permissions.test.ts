...
const threadPolicyStubUrl = `data:text/javascript,${encodeURIComponent(`
  export function canReplyToReviewThread({ audience, actorRole }) {
    return audience === "internal" || ["editor", "producer", "admin", "owner"].includes(actorRole);
  }

  export function replyAudienceFromParent({ visibility, reviewInviteId }) {
    return {
      visibility,
  ...
  export function replySourceFromParent({ timecodeSeconds }) {
    return {
...
registerHooks({
  resolve(specifier, context, nextResolve) {
    ...
    if (specifier === "@/lib/review/thread-policy") {
      return nextResolve(threadPolicyStubUrl, context);
    }
...
test("POST keeps an editor response inside the external version-bound thread", async () => {
  const { POST } = await routeModule();
  state.__ccoCommentParent = {
    ok: true,
    data: {
      ...
      asset_id: "asset-a",
      version_id: "version-a",
      review_invite_id: "invite-external",
      visibility: "external",
    },
  };

  const reviewerResponse = await POST(
    request("POST", { body: "Reviewer reply", parent_id: "external-parent" }),
    context,
  );
  assert.equal(reviewerResponse.status, 403);
  assert.equal(state.__ccoCommentSupabase?.inserts.length, 0);

  configure({ role: "editor", rank: 60 });
  state.__ccoCommentParent = {
    ok: true,
    data: {
      id: "external-parent",
  ...
  const editorResponse = await POST(
    request("POST", { body: "Producer response", parent_id: "external-parent" }),
    context,
  );
  assert.equal(editorResponse.status, 201);
  const write = state.__ccoCommentSupabase?.inserts[0];
  assert.equal(write?.payload.parent_id, "external-parent");
  assert.equal(write?.payload.version_id, "version-a");
  assert.equal(write?.payload.visibility, "external");
  assert.equal(write?.payload.review_invite_id, "invite-external");
  ...
  assert.equal(state.__ccoCommentSupabase?.inserts.length, 1);
});

test("POST derives every reply from its parent time and never creates a second pin", async () => {
  configure({ role: "editor", rank: 60 });
  state.__ccoCommentParent = {
    ok: true,
    data: {
  ...
  const response = await POST(
    request("POST", {
      body: "Reply without a duplicate marker",
      parent_id: "parent-a",
      version_id: "version-a",
      timecode_seconds: 99,
      pin_x: 0.75,
  ...
  assert.equal(response.status, 201);
  const write = state.__ccoCommentSupabase?.inserts[0];
  assert.equal(write?.payload.parent_id, "parent-a");
  assert.equal(write?.payload.timecode_seconds, 12.5);
  assert.equal(write?.payload.pin_x, null);
  assert.equal(write?.payload.pin_y, null);
});
