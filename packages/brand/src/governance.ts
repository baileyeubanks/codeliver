export const BRAND_GOVERNANCE_SCHEMA_VERSION = 1 as const;
export const BRAND_GOVERNANCE_STORAGE_KEY = "co-deliver.brand-governance.v1";

export type BrandScope = "platform" | "organization" | "workspace";
export type BrandRevisionStatus = "draft" | "published" | "archived";
export type BrandLogoPath = "/demo/cco-spiral.png" | "/demo/cco-lockup.png";
...
const LOGO_PATHS: BrandLogoPath[] = ["/demo/cco-spiral.png", "/demo/cco-lockup.png"];
...
export function resolveBrand(
  ...
  const published = (["platform", "organization", "workspace"] as BrandScope[])
    .map((scope) => latestRevision(revisions, context, scope))
  ...
  for (const revision of [...published, ...(preview ? [preview] : [])]) {
