...
async function handlePost(
  ...
  let command;
  try {
    command = parseProjectCallSheetDecisionRequest(body);
  } catch (error) {
  ...
  let result;
  try {
    result = await supabase.rpc("decide_project_call_sheet_revision", {
  ...
  const receipt = parseProjectCallSheetDecisionReceipt(result.data);
  if (
    !receipt ||
