...
test("the active production upload never publishes a raw storage URL before release readiness", () => {
  ...
  assert.match(uploader, /if \(!quarantined\) onUploadComplete\(\[\]\)/);
});
...
test("the resumable upload surface exposes readiness, progress, pause, retry, quarantine, and error states", () => {
  ...
  assert.match(uploader, /aria-label=\{`Pause upload for \$\{item\.file\.name\}`\}/);
  assert.match(uploader, /aria-label=\{`Retry upload for \$\{item\.file\.name\}`\}/);
  assert.match(uploader, /className="cockpit-upload-close"/);
});
...
test("the demo share sheet covers mobile navigation and serves local thumbnails directly", () => {
  const modal = source("components/demo/DemoShareModal.tsx");
  const styles = source("app/globals.css");

  assert.match(styles, /\.modal-overlay\.demo-share-overlay \{ z-index: 400 !important;/);
  assert.equal(modal.match(/\bunoptimized\b/g)?.length, 2);
});
