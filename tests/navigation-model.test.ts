...
test("workspace navigation is filtered by role capabilities", () => {
  ...
  assert.equal(roleCan("owner", "sales:qualify"), true);
  assert.equal(roleCan("producer", "sales:read"), true);
  assert.equal(roleCan("editor", "sales:read"), false);

  const viewerIds = visibleNavigation("viewer").flatMap((section) =>
    section.items.map((item) => item.id),
  );
  assert.deepEqual(viewerIds, ["projects", "reviews", "library"]);

  const ownerIds = visibleNavigation("owner").flatMap((section) =>
    section.items.map((item) => item.id),
  );
  assert.equal(ownerIds.includes("sales"), true);

  assert.equal(roleCan("client", "reviews:read"), true);
  assert.equal(roleCan("client", "projects:read"), false);
  assert.deepEqual(
    visibleNavigation("client").flatMap((section) =>
      section.items.map(({ id, label, href }) => ({ id, label, href })),
    ),
...
test("the most specific route wins active navigation", () => {
  ...
  assert.equal(activeNavigationId("/projects/archive", "owner"), "projects");
  assert.equal(activeNavigationId("/projects/trash/item-1", "owner"), "projects");
  assert.equal(activeNavigationId("/unknown", "owner"), null);
  assert.equal(activeNavigationId("/reviews", "client"), "reviews");
  assert.equal(activeNavigationId("/projects/ica", "client"), null);
});
