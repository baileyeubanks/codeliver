...
const DELIVERY_ID_PATTERN = /^[a-z0-9][a-z0-9._:-]{15,199}$/;
...
export function signWebhookPayload({
  ...
  const normalizedDeliveryId = deliveryId.trim().toLowerCase();
  if (!DELIVERY_ID_PATTERN.test(normalizedDeliveryId)) {
    throw new Error("Webhook delivery ID is invalid");
  }
  if (!Number.isInteger(attempt) || attempt < 1 || attempt > 12) {
    throw new Error("Webhook delivery attempt is invalid");
  }
  return `v2=${createHmac("sha256", secret)
    .update(
      `${timestamp}.${normalizedDeliveryId}.${attempt}.${body}`,
      "utf8",
    )
    .digest("hex")}`;
}

export function isRetryableWebhookResponse(responseCode: number) {
  return (
    responseCode === 408 ||
    responseCode === 425 ||
    responseCode === 429 ||
...
export async function deliverSignedWebhook({
  ...
  const response = await fetchImpl(safeUrl, {
    ...
    headers: {
      ...
      "X-Co-Production-Delivery-Id": normalizedDeliveryId,
      "X-Co-Production-Delivery-Attempt": String(attempt),
      "X-Co-Production-Signature": signature,
      "X-Co-Production-Signature-Version": "v2",
      "X-Co-Production-Timestamp": timestamp,
      "X-CoDeliver-Event": event,
      "X-CoDeliver-Delivery-Id": normalizedDeliveryId,
