...
async function handlePost(
  ...
  let command;
  try {
    command = parseProjectShotPlanSubmitRequest(body);
  } catch (error) {
  ...
  let result;
  try {
    result = await supabase.rpc("submit_project_shot_plan_revision", {
  ...
  const receipt = parseProjectShotPlanSubmitReceipt(result.data);
  if (
    !receipt ||
