...
async function handlePost(
  ...
  let command;
  try {
    command = parseProjectProductionScheduleSubmitRequest(body);
  } catch (error) {
  ...
  let result;
  try {
    result = await supabase.rpc("submit_project_production_schedule_revision", {
  ...
  const receipt = parseProjectProductionScheduleSubmitReceipt(result.data);
  if (
    !receipt ||
