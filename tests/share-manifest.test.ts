...
test("approval policy requires an exact step and recipient email, and cannot enable downloads", () => {
  const noStep = parseSingleShareRequest(
    single({ share_intent: "approval_needed", permissions: "approve" }),
    { authenticatedTenantId: "tenant-a", assetId: "asset-a", now: NOW },
  );
  assert.equal(noStep.ok, false);
  if (!noStep.ok) assert.match(noStep.error, /approval_id is required/);
  ...
  if (!noEmail.ok) assert.match(noEmail.error, /requires a recipient email/);
...
test("approval step identifiers are canonical manifest authority", () => {
  ...
  const second = parseSingleShareRequest(
    single({
      share_intent: "approval_needed",
      permissions: "approve",
  ...
  const misplaced = parseSingleShareRequest(
    single({ approval_id: "approval-a" }),
    { authenticatedTenantId: "tenant-a", assetId: "asset-a", now: NOW },
  );
  ...
  if (!first.ok || !second.ok || misplaced.ok) return;

  assert.equal(first.value.items[0].approvalId, "approval-a");
  assert.equal(second.value.items[0].approvalId, "approval-b");
  assert.notEqual(fingerprintShareManifest(first.value), fingerprintShareManifest(second.value));
  assert.match(misplaced.error, /only valid for approval-needed shares/);
});

test("optional share passwords are preserved exactly and bounded at the API boundary", () => {
  const accepted = parseSingleShareRequest(single({ password: "open door" }), {
    authenticatedTenantId: "tenant-a",
    assetId: "asset-a",
    now: NOW,
  });
  assert.equal(accepted.ok, true);
  if (accepted.ok) assert.equal(accepted.value.items[0].password, "open door");

  for (const password of ["1234567", "x".repeat(129), 12345678]) {
    const rejected = parseSingleShareRequest(single({ password }), {
      authenticatedTenantId: "tenant-a",
      assetId: "asset-a",
      now: NOW,
    });
    ...
    if (!rejected.ok) {
      assert.equal(rejected.field, "items[0].password");
      assert.match(rejected.error, /8-128 characters/);
    }
  }
});

test("manifest fingerprints bind only the password-protected state, never the secret", () => {
  const first = parseSingleShareRequest(single({ password: "first-passphrase" }), {
    authenticatedTenantId: "tenant-a",
    assetId: "asset-a",
    now: NOW,
  });
  const second = parseSingleShareRequest(single({ password: "second-passphrase" }), {
    authenticatedTenantId: "tenant-a",
    assetId: "asset-a",
    now: NOW,
  });
  const unprotected = parseSingleShareRequest(single(), {
    authenticatedTenantId: "tenant-a",
    assetId: "asset-a",
    now: NOW,
  });
  ...
  if (!first.ok || !second.ok || !unprotected.ok) return;

  assert.equal(fingerprintShareManifest(first.value), fingerprintShareManifest(second.value));
  assert.notEqual(fingerprintShareManifest(first.value), fingerprintShareManifest(unprotected.value));
});
