...
function recipientGroupFingerprint(
  key: string,
  items: PreparedShareManifest["items"],
) {
  const subject = JSON.stringify({
    recipient: JSON.parse(key),
    items: items
      .map((item) => ({ asset_id: item.assetId, version_id: item.version.id }))
      .sort((left, right) =>
        `${left.asset_id}:${left.version_id}`.localeCompare(
          `${right.asset_id}:${right.version_id}`,
        ),
      ),
  });
...
function scopedIdempotencyKey(base: unknown, key: string) {
...
export function buildShareNotificationRequests({
  ...
  | {
      ...
      requests: Array<{
        ...
        versionIds: string[];
        projectId: string;
        recipientGroupFingerprint: string;
      }>;
    }
  ...
  for (const [key, items] of groups) {
    ...
    requests.push({
      ...
      versionIds: items.map((item) => item.version.id),
      projectId: items[0].asset.project_id,
      recipientGroupFingerprint: recipientGroupFingerprint(key, items),
    });
  }
