...
## Front-door containment loop - 2026-07-15

- Password login now validates the exact server-controlled staff/client claim
  before a managed portal can accept the session. Pending and wrong-portal
  sessions are cleared locally; provider failures remain private.
- Email callbacks use the same cleanup rule. Pending, exchange-failure, and
  cleanup-failure states are visible on the shared login instead of silently
  returning to an unexplained form.
- Portal remediation uses the exact admin or client hostname and a sanitized
  local return path. It does not infer a role from the email string.
- The canonical `co-videopro.com` API gate now applies the authenticated role:
  client identities receive the minimal client allowlist, and staff identities
  receive the reviewed admin allowlist.
- The authenticated client workspace now lands on a dedicated `/reviews`
  inbox. Its shell exposes only Reviews, and every producer page and API remains
  denied. The inbox reads a bounded server DTO tied to the exact authenticated
  reviewer principal rather than widening project or asset authority.
- The client-principal and staff-surface migration is authored but unapplied.
  Until a controlled staging database proves direct client JWT denial and staff
  continuity, the source implementation is containment evidence, not launch
  authorization.
- The identity context route is staff/admin source-gated, but its database and
  provisioning authority remains unapplied and unproven.

Focused client inbox, host, navigation, public-review, and launch-gate checks
pass `57/57`. The complete suite passes `691/691`; TypeScript, repository
ESLint, and the native production build pass. No role provisioning, migration,
deployment, DNS, or public-site change was performed.
...
3. Critical: the client review inbox and restrictive staff-surface policy are
   implemented in source, but the migration is unapplied and has no real
   Postgres/RLS matrix proving direct client JWT denial and staff continuity.
4. High: audit records and feature exposure events are local and neither
   append-only nor independently retained.
5. High: brand assets are allowlisted demos; upload ownership, malware checks,
   transformations, CDN invalidation, and custom-domain proof are absent.
6. High: brand publication has no server concurrency guard, approval quorum,
   scheduled release, or cross-region rollback receipt.
7. Medium: localization is preference-ready but only English interface strings
   exist, and assistive-technology certification is not complete.
8. Medium: the selected in-app Browser backend is unavailable, so the client
   inbox, managed login states, and exact desktop/mobile shell still need fresh
   canonical visual and console evidence.
9. Medium: credential policy still inherits the existing six-character server
