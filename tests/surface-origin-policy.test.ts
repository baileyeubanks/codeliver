...
test("origin normalization accepts only origin-only HTTP(S) values", () => {
  assert.equal(
    normalizeSurfaceOrigin("  https://CO-VIDEOPRO.com:443/  ", "CLIENT_SITE_URL"),
    CLIENT_PRODUCTION_ORIGIN,
  );
  assert.equal(
  ...
  for (const value of [
    ...
    "//co-videopro.com",
    "ftp://co-videopro.com",
    "http://co-videopro.com",
    "https://user:password@co-videopro.com",
    "https://co-videopro.com/review/token",
    "https://co-videopro.com?next=https://attacker.test",
    "https://co-videopro.com#review",
    "https://co-videopro.com.",
    "https://co-videopro.com\\@attacker.test",
  ]) {
    assert.throws(
...
test("surface URL building rebases safe paths and rejects URL escape forms", () => {
  assert.equal(
    ...
    "https://co-videopro.com/review/token?download=1#media",
  );
  assert.equal(
    buildSurfaceUrl(
      ADMIN_PRODUCTION_ORIGIN,
    ...
    "https://co-videopro.com/projects/project-1/assets/asset-1",
  );
...
test("server env and email links use the canonical Co-VideoPro front door", async () => {
  await withSurfaceEnvironment(
    {
      NODE_ENV: "production",
    ...
    async () => {
      ...
      assert.match(
        comment.html,
        /href="https:\/\/co-videopro\.com\/projects\/project-1\/assets\/asset-1"/,
      );
    },
  );
