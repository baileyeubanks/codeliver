...
test("canonical, admin, and client hosts share one Content Co-op branded login", () => {
  ...
  assert.equal(resolveAuthHostContext(CANONICAL_HOST).label, "Co-VideoPro workspace");
  ...
  for (const host of [CANONICAL_HOST, ADMIN_HOST, CLIENT_HOST]) {
    assert.equal(
  ...
  const forgedOwnerEmail = {
    ...
    memberships: base.memberships.map((membership) =>
      membership.userId === "user-alex"
        ? { ...membership, email: "bailey@contentco-op.com" }
        : membership,
    ),
  };
...
test("return paths reject encoded auth loops, authority confusion, and control bytes", () => {
  ...
  assert.equal(isInviteReturnPath(invitePath), true);
  for (const unsafeInvitePath of [
    "/invite/short",
    `${invitePath}?role=admin`,
    `${invitePath}#session`,
    "/invite/%2Fprojects",
    "/invite/inviteToken_1234567890-ABCDEFGH/extra",
  ]) {
    assert.equal(isInviteReturnPath(unsafeInvitePath), false, unsafeInvitePath);
  }
});
...
test("protected admin and client cross-surface requests are denied without redirecting sessions", () => {
  ...
  assert.equal(resolveHostSurface(CANONICAL_HOST), "canonical");
