...
test("media library keeps one direct asset view and route-backed empty state", () => {
  const libraryPage = source("app/(dashboard)/library/page.tsx");

  assert.match(libraryPage, /Asset management/);
  assert.match(libraryPage, /Search the account library, then open the exact project cut you need/);
  assert.match(libraryPage, /\{filtered\.length\} \{filtered\.length === 1 \? "asset" : "assets"\}/);
