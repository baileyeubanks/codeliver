...
test("the local demo journey persists account, project, media, review, share, and settings state across reload", async () => {
  ...
  try {
    ...
    const readState = () => JSON.parse(values.get(key) ?? "null") as {
      ...
      reviewCutMarkers: Array<{ asset_id: string; version_id: string; time_seconds: number }>;
    ...
    workspace.addDemoReviewCutMarker({ projectId: project.id, assetId, versionId: "demo-version-1", timeSeconds: -2 });
    workspace.addDemoReviewCutMarker({ projectId: project.id, assetId, versionId: "demo-version-1", timeSeconds: 0.1 });
    workspace.addDemoReviewCutMarker({ projectId: project.id, assetId, versionId: "demo-version-1", timeSeconds: 8 });
    ...
    assert.deepEqual(
      beforeReload.reviewCutMarkers
        .filter((marker) => marker.asset_id === assetId && marker.version_id === "demo-version-1")
...
test("external approval authority is recipient-specific, sequential, and identity-redacted", async () => {
  ...
  const orderedStatuses = [...demoReviewPayload.approvals]
    .sort((left, right) => left.step_order - right.step_order)
    .map((approval) => approval.status);
  const firstPendingStep = orderedStatuses.indexOf("pending");
  assert.equal(firstPendingStep >= 0, true);
  assert.equal(
    orderedStatuses.slice(firstPendingStep).every((status) => status === "pending"),
    true,
    "a sequential workflow cannot record a later decision while an earlier step is pending",
  );
  assert.deepEqual(state.activeApprovalIds, ["approval-2"]);
  ...
  async () => {
    ...
    const authorized = await PATCH(
      new Request("http://localhost/api/review/demo/approvals", {
        ...
        body: JSON.stringify({ id: "approval-2", status: "approved" }),
      }),
    ...
    const foreignStep = await PATCH(
      new Request("http://localhost/api/review/demo/approvals", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: "approval-1", status: "rejected" }),
      }),
      { params: Promise.resolve({ token: "demo" }) },
    );
    assert.equal(
