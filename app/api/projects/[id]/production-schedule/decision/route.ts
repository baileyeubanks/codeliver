...
async function handlePost(
  ...
  try {
    projectId = normalizeProjectProductionScheduleUuid(
      (await params).id,
      "projectId",
    );
  } catch {
  ...
  if (
    ...
    declaredLength > PROJECT_PRODUCTION_SCHEDULE_COMMAND_MAX_BYTES
  ) {
  ...
  let command;
  try {
    command = parseProjectProductionScheduleDecisionRequest(body);
  } catch (error) {
  ...
  let result;
  try {
    result = await supabase.rpc("decide_project_production_schedule_revision", {
      p_project_id: projectId,
      p_expected_authority_version: command.expectedAuthorityVersion,
      p_production_schedule_revision_id: command.revisionId,
  ...
  if (result.error) {
    const mapped = classifyProjectProductionScheduleDatabaseError(result.error);
    return json({ error: mapped.error }, mapped.status);
  }
  const receipt = parseProjectProductionScheduleDecisionReceipt(result.data);
  if (
    !receipt ||
    receipt.projectId !== projectId ||
    receipt.productionScheduleRevisionId !== command.revisionId ||
