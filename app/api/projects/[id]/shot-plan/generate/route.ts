...
async function handlePost(
  ...
  try {
    projectId = normalizeProjectShotPlanUuid((await params).id, "projectId");
  } catch {
    return json({ error: "Project shot plan not found" }, 404);
  }
  ...
  if (
    ...
    declaredLength > PROJECT_SHOT_PLAN_COMMAND_MAX_BYTES
  ) {
    return json({ error: "Project shot plan request is too large" }, 413);
  }
  const raw = await request.text();
  if (Buffer.byteLength(raw, "utf8") > PROJECT_SHOT_PLAN_COMMAND_MAX_BYTES) {
    return json({ error: "Project shot plan request is too large" }, 413);
  }
  let body: unknown;
  try {
  ...
  let command;
  try {
    command = parseProjectShotPlanGenerateRequest(body);
  } catch (error) {
  ...
  let result;
  try {
    result = await supabase.rpc("generate_project_shot_plan_revision", {
      ...
      p_expected_production_plan_revision_id:
        command.expectedProductionPlanRevisionId,
  ...
  if (result.error) {
    const mapped = classifyProjectShotPlanDatabaseError(result.error);
    return json({ error: mapped.error }, mapped.status);
  }
  const receipt = parseProjectShotPlanGenerateReceipt(result.data);
  if (
    !receipt ||
    receipt.projectId !== projectId ||
    receipt.source.scriptRevisionId !== command.expectedScriptRevisionId ||
