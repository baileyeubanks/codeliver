...
async function handlePost(
  ...
  let command;
  try {
    command = parseProjectShotPlanDecisionRequest(body);
  } catch (error) {
  ...
  let result;
  try {
    result = await supabase.rpc("decide_project_shot_plan_revision", {
      p_project_id: projectId,
      p_expected_authority_version: command.expectedAuthorityVersion,
      p_shot_plan_revision_id: command.revisionId,
  ...
  const receipt = parseProjectShotPlanDecisionReceipt(result.data);
  if (
    !receipt ||
    receipt.projectId !== projectId ||
    receipt.shotPlanRevisionId !== command.revisionId ||
