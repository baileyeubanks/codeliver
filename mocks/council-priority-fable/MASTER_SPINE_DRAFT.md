# MASTER SPINE — draft from seat FABLE (rev. 3: after Grok 4.7, formal brief, Opus 5.5 rev 5.1)
...
| Nav master — phone bottom pipeline + one drawer; desktop thin left; shows Login → Projects, where Review / Approve / Preview sit, and where the "waiting on whom" home sits | **OPEN** | #3 (pick from mocks #30/#31/#32) → #10 (Land) |
| ACS quiet admin — roster (#2) → dispatch (#4) → Caio chip + crew list (#7) → close-out (#9) | **OPEN** | #2 → #4 → #7 → #9 |
...
| 7 | ACS-06 | P1 | ACS | **Caio's side of the job: confirmed chip + crew today-list.** Default states are minimal — **confirmed by / at, then done / issue** — tapped after the Continuity call. **Full states (en route / on-site) only if Bailey says Caio opens the crew surface in the van.** Rail stays voice/FaceTime at `+15048581959`; no new rail, no bot, no send. | Chip on the dispatch row (admin) and on the crew list; crew today-list legible in a van; no modals; **crew not paged; nothing sent.** **Ledger: crew list → PASS.** | Forge/Clip + Ring (owns the rail) | K (inputs: Bailey's crew-surface answer, evening routine) → G. No Council round. | #4 · B: crew-surface question | No — next cycle |
| 8 | CVP-03 | P1 | CVP | **Pilot:** per-deliverable, per-client status for **that one client** on the existing project, on `co_production.deliverables`, #5's columns mapped onto the review outcome from #6; **derived** from versions, rounds, locks. | Madeline does not retype that client's rows. Status strip on the existing cockpit (reuses the ACS list-with-chips idiom); no new chrome ahead of the pick. D8: derived, never hand-typed. | Reel; Latch | G | #5, #6 | No — next cycle |
| 9 | ACS-04 | P2 | ACS | **Close-out:** notes the office can invoice from, extra time, paid / unpaid **as a state** on the same Job. Client enrichment is **ROUTINE** (Caio's sequence via Ring/Blaze; code only if a Lupe field blocks a save). **Amanda held.** | Close flow ≤3 taps on phone; note and invoice state on the Job; **no invoice send.** ACS quiet held. **Ledger: ACS quiet (close-out) → PASS.** | Forge/Clip · ROUTINE: Ring/Blaze | G · ROUTINE | #7 | No |
| 10 | CVP-04 | P1 | CVP | **Cross-client "waiting on whom" home — where the Excel sheet dies.** Login → Projects; reads #8's derived status across all clients; every project shows its deliverable strip. | The picked master Landed: phone bottom pipeline + drawer, desktop thin left, logo PASS; no second nav; no left+bottom on phone. **Ledger: nav → PASS.** | Reel/Cut/Latch | G | #3 pick, #8 | No — next cycle, after #8 |
...
| 13 | CVP-05 | P2 | CVP | **Money on the job, CCO OS stays the commercial authority** (`20260812000000_commercial_handoff_fields.sql`: estimate line ↔ deliverable, frozen totals, "Co-VideoPro never mutates them"). CVP **shows** proposal + invoice state on the job and marks invoice-ready on lock. *Demo-runtime proposal promotion withdrawn — two price authorities means hand reconciliation.* | A delivered job holds a bill Bailey chose to send; **gate: one deliverable finished review and Bailey says bill.** Cockpit sections (D4), no finance app; no new payment rail. | Reel/Latch; Scout packs the CCO seam (input outside this repo) | K (seam) → G | #8; B: bill · B: does Madeline's sheet carry money? | No |
| 14 | KIMI-02 / ACS-08 | P2 (Land next cycle+) · K now | Both | **First FORM agents, both siblings, one artifact-card idiom.** CVP: **QC vs approved brief + chase list** on the open round (reads brief + version + open comments; script scaffolds later). ACS: **close-note drafts + roster hygiene suggestions** (reads only records ACS owns). **No outbound drafts** — CS reply drafts wait until the no-send rule is proven over a real week. (Blaze: Copilot-in-ACS is vault-history intent; narrowed accordingly.) | Operator accepts or rejects each named artifact. Nothing chats, spends, or sends. Same card in the CVP rail/drawer and in a quiet ACS review queue. | Frame/Scout (one idiom pack); Cut / Forge per train | K after #5; G per train on its own clock | CVP: #6 + one real finished round · ACS: #9 (may run first) | Pack only |
| 15 | HOLD | P3 | Both | WEFTEC · phone CS bot and any outbound CS drafts · Amanda enrich (this cycle) · Wipster hosting exit / archive migration · player / overlay / logo / auth redo · drawing suite · transcript NLE · **migration application (Bailey gate)** · re-opening CCNAS / scan / approval setup without a live regression · **landing PR #25 as-is**. | Nobody starts these. | — | — | No |
...
| B7 | Does Caio (or the crew) open the crew surface in the van? | **WAITING on Bailey** | Row 7 stays minimal states (confirmed / done / issue) by default; full states only on a yes. Row 7 is next cycle, so this does not block the lock. |
...
- #6 starts when #1 is proved **and** Bailey has picked #3. It is a reconcile of PR #25's VA-018, not a build, and it is **not blocked on #5** (finish writes the version's review outcome; #5 maps onto it). If the pick is late, the Grok lane stays on ACS (#4, then #7 next cycle).
...
|---|---|---|
| #1, #2 | #5 Madeline columns · #4 VA-106 failure note · #6 share-mode **placement** against the #3 pick | VA sweep of live ACS roster after #2. *(Reality-refresh pack dropped: Grok and Opus read the open PRs in preflight — packs are inputs only.)* |
| #4 | #7 Caio-chip + crew-list inputs (Bailey's crew-surface answer, evening routine) · #8 mapping onto `co_production.deliverables` | VA sweep of dispatch after #4; nav pick checklist |
| #6 | #9 close-out inputs · #10 "waiting on whom" Land checklist | post-#6 proof: a round ended and the outcome landed on the version |
