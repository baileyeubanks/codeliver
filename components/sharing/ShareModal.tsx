...
function summarizeNotificationStatus(
  ...
  if (result.mode === "queued") return "queued";
  ...
  if (
    ...
    statuses.every((status) => status === "queued" || status === "sent")
  ) {
    return "queued";
  }
...
function ShareModalContent({
  ...
  const [approvalSteps, setApprovalSteps] = useState<ApprovalStepOption[]>([]);
  const [approvalStepsLoading, setApprovalStepsLoading] = useState(false);
  ...
  const selectedApprovalStep =
    approvalSteps.find((step) => step.id === approvalId) ?? null;
  ...
  useEffect(() => {
    if (previewMode || shareIntent !== "approval_needed" || !versionId) {
      setApprovalSteps([]);
      setApprovalStepsLoading(false);
      setApprovalId("");
      return;
    }

    let cancelled = false;
    setApprovalStepsLoading(true);
    setApprovalId("");
    fetch(`/api/assets/${assetId}/approvals?version_id=${encodeURIComponent(versionId)}`)
      .then((response) =>
        response.ok ? response.json() : Promise.reject(new Error("Approval step load failed")),
      )
      .then((data) => {
        if (cancelled) return;
        const steps = Array.isArray(data.items)
          ? (data.items as ApprovalStepOption[]).filter(
              (step) => step.status === "pending" && Boolean(step.assignee_email),
            )
          : [];
        setApprovalSteps(steps);
        const firstStep = steps[0] ?? null;
        setApprovalId(firstStep?.id ?? "");
        setReviewerEmail(firstStep?.assignee_email ?? "");
        invalidateLiveSendAuthority();
        if (!firstStep) {
          setError("Create a pending approval step for this version before sharing an approval link.");
        }
      })
      .catch(() => {
        if (!cancelled) {
          setApprovalSteps([]);
          setError("Approval steps could not be loaded for this version.");
        }
      })
      .finally(() => {
        if (!cancelled) setApprovalStepsLoading(false);
      });

    return () => {
      cancelled = true;
    };
  }, [assetId, previewMode, shareIntent, versionId]);
  ...
  async function submitShare(operation: "preview" | "create") {
    ...
    if (!previewMode && shareIntent === "approval_needed" && !approvalId) {
      setError("Choose the exact approval step for this link.");
      return;
    }
    if (
      requirePassword &&
      (password.length < SHARE_PASSWORD_MIN_LENGTH || password.length > SHARE_PASSWORD_MAX_LENGTH)
    ) {
      setError(
        `Password must be ${SHARE_PASSWORD_MIN_LENGTH}-${SHARE_PASSWORD_MAX_LENGTH} characters.`,
      );
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`/api/assets/${assetId}/share`, {
        ...
        body: JSON.stringify({
          ...
          approval_id: shareIntent === "approval_needed" ? approvalId || null : null,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 py-6">
      <div className="max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-[var(--radius)] border border-[var(--border)] bg-[var(--surface)] shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-[var(--border)] px-6 py-5">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2 text-xs text-[var(--muted)]">
              <span className="rounded-full bg-[var(--surface-2)] px-3 py-1">Co-VideoPro sharing</span>
              {statusLabel ? (
                ...
                <div className="rounded-[var(--radius)] border border-[var(--border)] bg-[var(--bg)] px-4 py-4">
                  ...
                  <div className="mt-4 grid gap-3">
                    ...
                    {shareIntent === "approval_needed" ? (
                      <div>
                        ...
                        <select
                          id="approval-step"
                          value={approvalId}
                          disabled={previewMode || approvalStepsLoading || approvalSteps.length === 0}
                          onChange={(event) => {
                            const nextStep = approvalSteps.find(
                              (step) => step.id === event.target.value,
                            );
                            setApprovalId(event.target.value);
                            setReviewerEmail(nextStep?.assignee_email ?? "");
                            invalidateLiveSendAuthority();
                          }}
                          className="w-full rounded-[var(--radius-sm)] border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm text-[var(--ink)] outline-none focus:border-[var(--accent)] disabled:opacity-50"
                        >
                          {previewMode ? <option value="">Demo approval route</option> : null}
                          {!previewMode && approvalSteps.length === 0 ? (
                            <option value="">
                              {approvalStepsLoading ? "Loading approval steps..." : "No pending approval step"}
                            </option>
                          ) : null}
                        ...
                        <p className="mt-2 text-xs leading-5 text-[var(--dim)]">
                          {selectedApprovalStep?.assignee_email
                            ? `This link is restricted to ${selectedApprovalStep.assignee_email}.`
                            : "A live approval link must be bound to one pending step."}
                        </p>
                      </div>
                    ) : null}
                    ...
                    <div>
                      ...
                      <input
                        value={reviewerEmail}
                        readOnly={shareIntent === "approval_needed" && !previewMode}
                        onChange={(event) => {
                          if (shareIntent === "approval_needed" && !previewMode) return;
                      ...
                      <p className="mt-2 text-xs leading-5 text-[var(--dim)]">
                        {intentDefaults.requiresReviewerEmail
                          ? "The approval step sets this recipient so the link cannot decide another step."
                      ...
                      {requirePassword ? (
                        <div>
                          ...
                          <input
                            ...
                            placeholder={`${SHARE_PASSWORD_MIN_LENGTH}-${SHARE_PASSWORD_MAX_LENGTH} characters`}
                            className="w-full rounded-[var(--radius-sm)] border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm text-[var(--ink)] outline-none transition-colors placeholder:text-[var(--dim)] focus:border-[var(--accent)]"
                          />
                        </div>
                      ) : null}
                ...
                <div className="grid gap-2 sm:grid-cols-[auto_1fr]">
                  <button
                    ...
                    disabled={
                      ...
                      (!previewMode &&
                        shareIntent === "approval_needed" &&
                        (approvalStepsLoading || !approvalId))
                    }
                ...
                <div className="mt-4 space-y-4 text-sm leading-6 text-[var(--muted)]">
                  ...
                  <p>
                    {notificationStatus === "queued"
                      ? "The link was created and the requested notifications were accepted for durable delivery. Delivery is pending."
