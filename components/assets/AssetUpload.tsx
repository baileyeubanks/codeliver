...
export default function AssetUpload({
    ...
    (item: UploadItem) => {
      ...
      const upload = new tus.Upload(item.file, {
        ...
        onSuccess() {
          ...
          if (!quarantined) onUploadComplete([]);
        },
        onError(error) {
  ...
  const cancelUpload = useCallback(
    async (id: string) => {
      ...
      try {
        if (item.tusUpload) {
          await item.tusUpload.abort(true); // true = terminate on server
        }
        ...
        updateItem(id, {
          cancelling: false,
          cancellationError:
            "Cancellation could not be confirmed. The upload remains in the queue; retry cancellation.",
        });
      }
    },
    [items, updateItem],
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-xs text-[var(--dim)] ml-2 flex-shrink-0 whitespace-nowrap">
                    ...
                    >
                      {item.cancelling ? "Cancelling..." : statusLabel(item.status, item.progress)}
                    </span>
                  </span>
                </div>
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                {item.cancelling ? (
                  <div
                    ...
                    role="status"
                    aria-label={`Cancelling upload for ${item.file.name}`}
                  />
                ) : item.cancellationError ? (
                  <button
                    type="button"
                    ...
                    aria-label={`Retry cancellation for ${item.file.name}`}
                  >
                    <RotateCcw size={16} />
                  </button>
                ) : item.status === "done" ? (
                  ...
                  <>
                    ...
                    <button
                      ...
                      aria-label={`Cancel quarantined upload for ${item.file.name}`}
                    >
                      <X size={16} />
                    </button>
                  </>
                  ...
                  <>
                    ...
                    <button
                      ...
                      className="text-[var(--dim)] hover:text-[var(--red)]"
                      title="Remove"
                      aria-label={`Remove failed upload for ${item.file.name}`}
                    >
                      <X size={16} />
                    </button>
                  </>
                  ...
                  <>
                    <button
                      ...
                      className="text-[var(--dim)] hover:text-[var(--accent)]"
                      title="Pause upload"
                      aria-label={`Pause upload for ${item.file.name}`}
                    >
                      <Pause size={14} />
                    </button>
                    <button
                  ...
                  <>
                    <button
                      ...
                      className="text-[var(--dim)] hover:text-[var(--accent)]"
                      title="Resume upload"
                      aria-label={`Resume upload for ${item.file.name}`}
                    >
                      <Play size={14} />
                    </button>
                    <button
                  ...
                  <button
                    ...
                    className="text-[var(--dim)] hover:text-[var(--red)]"
                    title="Cancel"
                    aria-label={`Cancel pending upload for ${item.file.name}`}
                  >
                    <X size={16} />
                  </button>
                )}
