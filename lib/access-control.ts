...
export async function getProjectAccess(
  ...
  const member = await database
    ...
    .maybeSingle();
  if (member.error) {
    return { ok: false, status: 500, error: member.error.message };
  }
  if (
    member.data &&
  ...
  if (project.team_id) {
    ...
    if (team.error || teamMember.error) {
      return {
        ok: false,
        status: 500,
        error: team.error?.message ?? teamMember.error?.message ?? "Access lookup failed",
      };
    }
...
export async function getAssetComment(
  ...
  const { data, error } = await db(client)
    .from("comments")
    .select("id, asset_id, version_id, visibility")
    .eq("id", commentId)
    .eq("asset_id", assetId)
    .maybeSingle();
