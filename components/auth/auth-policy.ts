...
> = {
  admin: {
    label: "Content Co-op team portal",
    origin: "https://admin.contentco-op.com",
  },
  client: {
    label: "client review portal",
...
export function resolveAuthStatusNotice(search: string): AuthStatusNotice | null {
  const params = new URLSearchParams(search);
  const accessValues = params.getAll("access");
  const authErrorValues = params.getAll("auth_error");

  if (
    accessValues.length === 1 &&
    accessValues[0] === "pending" &&
