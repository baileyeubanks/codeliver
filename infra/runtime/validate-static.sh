...
if /usr/bin/grep -Eq '^(NEXT_PUBLIC_SUPABASE_ANON_KEY|SUPABASE_SERVICE_KEY|CO_PRODUCTION_TOKEN_ENCRYPTION_KEY|CO_PRODUCTION_WEBHOOK_SECRET_ENCRYPTION_KEY|CO_PRODUCTION_ANALYTICS_HASH_KEY|CO_PRODUCTION_REVIEW_ADMISSION_SIGNING_KEY|CO_PRODUCTION_REVIEW_ADMISSION_VERIFICATION_KEYS|RESEND_API_KEY|ANTHROPIC_API_KEY|CODELIVER_MEDIA_PIPELINE_WORKER_TOKEN|NOTIFICATION_WEBHOOK_SECRET)=.+' "$ENV_TEMPLATE"; then
