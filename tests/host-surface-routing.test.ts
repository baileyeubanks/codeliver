...
test("managed hosts are recognized exactly without suffix or credential confusion", () => {
  assert.equal(resolveHostSurface(CANONICAL_PRODUCT_HOST), "canonical");
  assert.equal(resolveHostSurface(CANONICAL_PRODUCT_WWW_HOST), "canonical");
...
test("role routing is explicit and fails closed", () => {
  ...
  assert.equal(roleCanAccessSurface("staff", "canonical"), true);
  assert.equal(roleCanAccessSurface("client", "canonical"), true);
  assert.equal(roleCanAccessSurface(null, "canonical"), false);
...
test("client workspace page policy exposes only the review inbox", () => {
  assert.deepEqual(clientWorkspacePathDecision("/reviews"), { action: "allow" });
  assert.deepEqual(clientWorkspacePathDecision("/reviews/"), { action: "allow" });

  for (const pathname of ["/", "/projects", "/projects/"]) {
    assert.deepEqual(clientWorkspacePathDecision(pathname), {
      action: "redirect",
      pathname: "/reviews",
    });
  }
  ...
  for (const pathname of [
    ...
    assert.deepEqual(clientWorkspacePathDecision(pathname), { action: "deny" });
  }
});
...
test("one login path carries only a safe local return target", () => {
  ...
  assert.equal(
    buildSurfaceUrl("canonical", "/projects/ica?demo=1").toString(),
    "https://co-videopro.com/projects/ica?demo=1",
  );
});
...
test("proxy routes verified identities and denies untrusted managed-surface access", async () => {
  ...
  try {
    ...
    const clientAllowed = await proxy(
      new NextRequest("https://client.contentco-op.com/reviews", {
    ...
    for (const pathname of ["/", "/projects"]) {
      const redirected = await proxy(
        new NextRequest(`https://${CLIENT_SURFACE_HOST}${pathname}`, {
          headers: { host: CLIENT_SURFACE_HOST },
        }),
      );
      assert.equal(redirected.status, 307, pathname);
      assert.equal(
        redirected.headers.get("location"),
        `https://${CLIENT_SURFACE_HOST}/reviews`,
        pathname,
      );
    }

    for (const pathname of ["/projects/ica", "/library", "/settings"]) {
      const clientPageDenied = await proxy(
        new NextRequest(`https://${CLIENT_SURFACE_HOST}${pathname}`, {
          headers: { host: CLIENT_SURFACE_HOST },
        }),
      );
      assert.equal(clientPageDenied.status, 403, pathname);
      assert.equal(clientPageDenied.headers.get("location"), null, pathname);
      assert.match(
        await clientPageDenied.text(),
        /signed-in account is not authorized for this Co-VideoPro surface/i,
        pathname,
      );
    }
    ...
    assert.match(await unknownHost.text(), /hostname is not an approved Co-VideoPro surface/i);
    ...
    for (const unsupportedHost of unsupportedHosts) {
      for (const path of publicAndAssetPaths) {
        ...
        if (path.startsWith("/api/")) {
          ...
          assert.match(
            await response.text(),
            /hostname is not an approved Co-VideoPro surface/i,
          );
        }
      }
    }
    ...
    for (const surfaceHost of [ADMIN_SURFACE_HOST, CLIENT_SURFACE_HOST]) {
      for (const { path, method } of [
        ...
        { path: "/demo/cco-lockup.png", method: "GET" },
        { path: "/brand/co-production-pro-horizontal.png", method: "GET" },
        { path: "/fonts/inter-latin.woff2", method: "GET" },
      ]) {
        const response = await proxy(
