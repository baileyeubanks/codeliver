...
async function handlePost(
  ...
  let command;
  try {
    command = parseProjectProductionScheduleGenerateRequest(body);
  } catch (error) {
    ...
    result = await supabase.rpc(
      "generate_project_production_schedule_revision",
      {
        ...
        p_expected_shot_plan_revision_id:
          command.expectedShotPlanRevisionId,
  ...
  const receipt = parseProjectProductionScheduleGenerateReceipt(result.data);
  if (
    !receipt ||
    receipt.projectId !== projectId ||
    receipt.source.shotPlanRevisionId !==
      command.expectedShotPlanRevisionId ||
