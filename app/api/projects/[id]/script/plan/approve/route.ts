...
async function handlePost(
  ...
  const command = parseProjectScriptPlanApprovalCommand(body);
  if (!command) return json({ error: "Script production plan request is invalid" }, 422);

  let result;
  try {
    result = await supabase.rpc("approve_project_script_plan_draft", {
      p_project_id: projectId,
      p_draft_id: command.draftId,
      p_expected_plan_revision: command.expectedPlanRevision,
  ...
  const receipt = parseProjectScriptPlanApprovalReceipt(result.data);
  if (
    !receipt
    || receipt.projectId !== projectId
    || receipt.draftId !== command.draftId
