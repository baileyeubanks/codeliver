...
async function multipartRequest(
  ...
  const workerToken = process.env.CODELIVER_MEDIA_PIPELINE_WORKER_TOKEN;
  if (workerToken) {
    headers.set("x-codeliver-media-worker-token", workerToken);
  }
...
test("legacy uploads sanitize traversal names and never overwrite on races", async () => {
  ...
  const previousRoot = setRoot(root);
  const previousWorkerToken = process.env.CODELIVER_MEDIA_PIPELINE_WORKER_TOKEN;
  process.env.CODELIVER_MEDIA_PIPELINE_WORKER_TOKEN = "legacy-media-test-worker-token";
  state.__legacyMediaRouteUser = {
    app_metadata: { content_coop_role: "staff" },
  };
  ...
  try {
    ...
    if (previousWorkerToken === undefined) {
      ...
      process.env.CODELIVER_MEDIA_PIPELINE_WORKER_TOKEN = previousWorkerToken;
    }
    restoreRoot(previousRoot);
    state.__legacyMediaRouteUser = null;
    rmSync(root, { recursive: true, force: true });
